$(document).ready(function () {
    ...
    // --- Compliments Array (Father's Day Themed) ---
    const compliments = [
    ...
    const startCardAnimations = () => {
        ...
        // --- Falling "Petals" (now blue/purple confetti/stars) ---
        ...
        setInterval(() => {
            ...
            const comp = $('<div class="compliment"></div>').html(msg); // Use .html() to render emojis
            const bottomStart = isMobile ? '12vh' : '8vh';
        ...
        $(document).on('mousemove', function (e) {
            ...
            // Cloud Parallax
            const x = (e.clientX / window.innerWidth - 0.5) * (isMobile ? 15 : 30);
            const y = (e.clientY / window.innerHeight - 0.5) * (isMobile ? 5 : 10);
            $('.cloud').each(function (i) {
                const depth = Math.max(1, (i % 3) + 1);
    ...
    const createFloatingHearts = () => {
        ...
        for (let i = 0; i < numHearts; i++) {
            ...
            for (let attempts = 0; attempts < 10; attempts++) {
                leftPos = Math.random() * (window.innerWidth - size - 20) + 10;
                const isTooClose = usedPositions.some(pos => Math.abs(pos - leftPos) < size + 10);
                if (!isTooClose || attempts === 9) {
                    usedPositions.push(leftPos);
                    if (usedPositions.length > 5) usedPositions.shift();
                    break;
                }
            }
    ...
    const initiateExperience = () => {
        ...
        $(document).off('click.initiateMobile');
